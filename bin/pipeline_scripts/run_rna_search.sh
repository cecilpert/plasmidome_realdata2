...
function verif_args(){
	...
	verif_file $rna_db 	"[rna_search] RNA database doesn't found in $rna_db" "[rna_search] RNA database found in $assembly"
	mkdir -p $outdir 
	if [[ ! $prefix ]]; then 
...
if [[ $file_exist == 1 ]]; then 
	...
	bash $BIN2/parallelize_blast.sh $rna_db $assembly $out.blast.tsv 32 blastn
